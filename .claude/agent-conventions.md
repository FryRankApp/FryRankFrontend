...
Shared house rules for every FryRankFrontend subagent. Read this at the start of any task. Your own agent definition layers role-specific rules on top of these; where a role rule is more specific, it wins.
...
- **Verify backend contracts against the live backend.** When a question touches an endpoint shape, open `../FryRankLambda` and read the current handler, model class, and `QueryParam`-style enum. Never trust memory, skill docs, or this repo's constants alone — drift between frontend and backend constants is itself a bug worth reporting.
...
- **Implementation happens in the main session.** This team has no engineer agent for now — the user implements hands-on with the main session. Hand findings and recommendations back; don't produce finished code diffs unasked.
